...
# 2014-Aug-28: More fixups for Solaris x86/64, because some files are missing from the package that allow
#              loading of the load objects and mapping the callstack. Added the version header (see below) and
#              copy this script to the extras folder so we know which version created the package.
#
#              ADDED UPDATEDATE STRING to be like Linux version
#
#
#####

#Added UPDATEDATE string
UPDATEDATE="2014.08.28 11:38 PDT"

/bin/echo "Oracle/Sun Solaris pkgcore.sh" 
/bin/echo "   with Actian/VOD modifications of ${UPDATEDATE}"
/bin/echo "   supporting Solaris 8, 9, 10 and Solaris/x86"
/bin/echo " "
...
# Grab the files specifically required for 64bit Solaris/x86
if /usr/bin/uname -p | grep 'i386' 2>&1 >/dev/null
   then
	if /usr/bin/test \(  $SOLARISVERSION -gt 9 \)
	  then
        # 32bit versions and their symlink targets
		ls /usr/lib/ld.so.1  >> ${UNIQUEBASE}/request.list
		ls /usr/lib/librtld_db.so.1  >> ${UNIQUEBASE}/request.list
		ls /lib/ld.so.1  >> ${UNIQUEBASE}/request.list
		ls /lib/librtld_db.so.1  >> ${UNIQUEBASE}/request.list
		ls /usr/lib/libthread_db.so.1  >> ${UNIQUEBASE}/request.list
		ls /lib/libc_db.so.1  >> ${UNIQUEBASE}/request.list
        
      	# 64bit versions and their symlink targets
		ls /usr/lib/amd64/ld.so.1  >> ${UNIQUEBASE}/request.list
		ls /usr/lib/64/ld.so.1  >> ${UNIQUEBASE}/request.list
		ls /usr/lib/amd64/librtld_db.so.1  >> ${UNIQUEBASE}/request.list
		ls /usr/lib/64/librtld_db.so.1  >> ${UNIQUEBASE}/request.list
		ls /lib/amd64/ld.so.1  >> ${UNIQUEBASE}/request.list
		ls /lib/64/ld.so.1  >> ${UNIQUEBASE}/request.list
		ls /lib/amd64/librtld_db.so.1  >> ${UNIQUEBASE}/request.list
		ls /lib/64/librtld_db.so.1  >> ${UNIQUEBASE}/request.list
		ls /usr/lib/amd64/libthread_db.so.1  >> ${UNIQUEBASE}/request.list
		ls /usr/lib/64/libthread_db.so.1  >> ${UNIQUEBASE}/request.list
		ls /lib/amd64/libc_db.so.1  >> ${UNIQUEBASE}/request.list
		ls /lib/64/libc_db.so.1  >> ${UNIQUEBASE}/request.list
	fi
fi
...
# the stack and heap (or so it seems)
	if /usr/bin/uname -p | grep 'sparc' 2>&1 >/dev/null
	  then
		...
		/bin/echo "Trying to find executable from pmap output"
		...
		if /usr/bin/test \( `/bin/echo ${EXEFILE} | wc -w ` -ne 1 \)
		  then
			/bin/echo "Trying alternate method to find executable from pmap output"
			# alternate: exefile is above heap
			EXEFILE=`pmap $COREFILE | /usr/bin/head -${BOTTOMLINE} | /usr/bin/tail -1 | /usr/bin/awk '{ print $4 } '`
			EXEFILE=`echo ${EXEFILE} | xargs -n 1 | uniq`
			if /usr/bin/test \( `/bin/echo ${EXEFILE} | wc -w ` -ne 1 \)
			  then
...
# Remove duplicates from request.list
/bin/mv -f ${UNIQUEBASE}/request.list ${UNIQUEBASE}/request.orig
/bin/sort ${UNIQUEBASE}/request.orig | /bin/uniq > ${UNIQUEBASE}/request.list
/bin/rm -f ${UNIQUEBASE}/request.orig
...
# Copy self to the extras folder
